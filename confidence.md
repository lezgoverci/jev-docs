---
source_url: https://docs.typesafe.ai/confidence.md
fetched_at: 2026-09-21
local_path: confidence.md
---
...
### Example Probability Distributions and Confidence

Below are example probability distributions across three options (`A`, `B`, `C`) and the resulting confidence scores calculated by TypeSafe:

| Distribution Pattern | Probabilities (A, B, C) | Selected Option | Confidence | Interpretation |
| :--- | :--- | :--- | :--- | :--- |
| **Clear winner** | 90%, 6%, 4% | Option A | **0.85** | High certainty — act automatically |
| **Spread out** | 40%, 33%, 27% | Option A | **0.10** | Low certainty — proceed with caution |
| **Even split** | 33.3%, 33.3%, 33.3% | Tie: A, B, C | **0.00** | Pure uncertainty — do not act |

<details>
<summary><strong>How TypeSafe calculates Confidence</strong></summary>

TypeSafe computes confidence from how the probability is distributed across the options:
- When probability is concentrated entirely on one option, confidence is `1.0`.
- The more evenly it spreads across alternatives, the lower the confidence.
- For a three-option question, confidence approximates:
  $\text{confidence} = \frac{3 \times \text{peak probability} - 1}{2}$

*(An interactive slider version of this explorer is available on the live web documentation at [docs.typesafe.ai/confidence](https://docs.typesafe.ai/confidence)).*
</details>
